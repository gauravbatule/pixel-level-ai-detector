...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  // Authentic optical camera check: genuine optical sensor noise
  const isSyntheticChroma = chroma.rRB < 0.965 && avgNoiseVar > 450;
  const isAuthenticCamera = !isSyntheticChroma && (bnc < -0.05 || (!isLossless && chroma.rRB >= 0.970) || (avgNoiseVar < 280 && editRatio < 0.05));

  // Digital UI Screenshot check: flat solid color UI surfaces or crisp text on solid canvas
  const isDigitalUIScreenshot = (exactFlatRatio > 0.50 || (isLossless && exactFlatRatio > 0.30 && editRatio < 0.035)) && !hasModal;
  ...
  if (!isDigitalUIScreenshot && !isAuthenticCamera) {
    ...
    } else if (chroma.rRB < 0.965 && avgNoiseVar > 450) {
      // Cross-channel residual decoupling with high synthetic noise
      fullAIScore = 0.88;
  ...
  if (editRatio > (mode === 'strict' ? 0.02 : mode === 'normal' ? 0.04 : 0.03)) {
    ...
    if ((isAuthenticCamera || isDigitalUIScreenshot) && editRatio < 0.25 && fullAIScore < 0.5 && !hasModal) {
      pixelScore *= 0.15;
    }
  } else {
  ...
  const isLocalInpainted = (editRatio > 0.035 && pixelScore > 0.30) || pixelScore > 0.35 || hasModal;
...
async function runAll() {
  console.log("==========================================================================================");
  console.log("  COMPLETE DATASET BENCHMARK EVALUATION (PHOTOS + SCREENSHOTS + FULL AI + INPAINT)");
  console.log("==========================================================================================\n");
  ...
  console.log("\n--- 3. FULL AI GENERATED IMAGES (Target: >= 70%) ---");
  const s3 = await testFolder('test_dataset/ai_full_generated', true);

  console.log("\n--- 4. AI INPAINTED & COMPOSITED IMAGES (Target: >= 70%) ---");
