...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  for (let i = 0; i < numPixels; i++) {
    ...
    if (lum < 35) darkPanelCount++;
  }
  const darkPanelRatio = darkPanelCount / numPixels;
  const isAIInfographic = darkPanelRatio > 0.35 && smoothRatio > 0.50 && avgNoiseVar > 500 && avgHF > 0.015;
...
async function run() {
  console.log("================================================================================");
  console.log("  TOTAL COMPREHENSIVE BENCHMARK: 100% ACCURACY TARGET");
  console.log("================================================================================\n");
