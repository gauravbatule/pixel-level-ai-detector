...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  const isDigitalUIScreenshot = exactFlatRatio > 0.60;
  ...
  // Authentic optical camera check: genuine low-variance photon shot noise (avgNoiseVar < 250) or bnc < -0.05
  const isAuthenticCamera = (avgNoiseVar < 280 && editRatio < 0.12) || (bnc < -0.05 && avgNoiseVar < 400 && editRatio < 0.12);
