...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  // 2. Strong chromatic decoupled residual (universal across all formats)
  else if (chroma.rRB < 0.965) {
    fullAIScore = 0.88;
  }
  // 3. AI dimensions + extreme latent smoothing (>50% smooth blocks) + high frequency deficit
  else if (matchesAIDim && smoothRatio > 0.50 && avgHF < 0.03) {
    fullAIScore = 0.88;
  }
...
async function run() {
  console.log("================================================================================");
  console.log("  TOTAL BENCHMARK (1536x1024 WhatsApp AI Image Full Verification)");
  console.log("================================================================================\n");
  ...
  for (const item of ALL_CASES) {
    ...
    const maxDim = 1920; // Exact browser dimension logic
