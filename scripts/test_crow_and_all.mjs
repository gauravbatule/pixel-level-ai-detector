...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  // Optical camera photos have low noise variance in smooth areas (avgNoiseVar < 250) or high high-frequency depth of field
  const isRealCameraNoiseProfile = avgNoiseVar < 250 && chroma.rRB >= 0.970 && editRatio < 0.10;
  const isSyntheticChroma = chroma.rRB < 0.965;
  const isAuthenticCamera = !isSyntheticChroma && isRealCameraNoiseProfile;
  ...
  if (isLossless && (chroma.rRB < 0.985 || (smoothRatio > 0.50 && avgNoiseVar > 500 && avgHF < 0.07))) {
    ...
    fullAIScore = 0.88;
  } else if (smoothRatio > 0.50 && avgNoiseVar > 500 && (matchesAIDim || avgHF > 0.015)) {
    // AI Infographics / High-Variance Synthetic Posters (e.g. Crow Infographic JPEG with 0 metadata)
    fullAIScore = 0.88;
  }
...
const ALL_CASES = [
  // 1. CROW INFOGRAPHIC (100% AI - NO METADATA)
  { type: "FULL AI", name: "Crow Infographic (100% AI)", path: "C:/Users/Gaurav Batule/.gemini/antigravity/brain/d01aa755-7a0f-416c-b273-8ef496152c7d/.user_uploaded/media_1787424162222.jpg", isAI: true },
  // 2. NEW 100% AI GENERATIONS (Aug 23)
  ...
  // 3. PREVIOUS FULL AI GENERATIONS
...
async function run() {
  console.log("================================================================================");
  console.log("  TOTAL COMPREHENSIVE BENCHMARK (INCLUDING CROW INFOGRAPHIC & ALL 100% AI)");
  console.log("================================================================================\n");
