...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  const isDigitalUIScreenshot = exactFlatRatio > 0.50 || (isLossless && exactFlatRatio > 0.30 && editRatio < 0.035 && avgSusp < 0.07);
  ...
  if (editRatio > (mode === 'strict' ? 0.02 : mode === 'normal' ? 0.04 : 0.03)) {
    ...
    if (isAuthenticCamera && editRatio < 0.08 && avgSusp < 0.08 && fullAIScore < 0.5) {
      pixelScore *= 0.15;
    } else if (isDigitalUIScreenshot && editRatio < 0.05 && fullAIScore < 0.5) {
      pixelScore *= 0.15;
    }
  } else {
