...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, width, height, isLossless = false, mode = 'balanced') {
  ...
  // Authentic physical camera sensor detection (Poisson shot noise physics)
  const isAuthenticCamera = bnc < -0.08;
  ...
  // Full AI Generation Detection (Diffusion / GAN models)
  // AI synthesis features:
  // 1. Lossless PNG with decoupled chroma residuals: rRB < 0.985 (vs 0.995+ in real PNG/JPEG)
  // 2. High smooth ratio across canvas (smoothRatio > 0.40) combined with non-camera noise (BNC >= -0.05)
  // 3. Spectral high-frequency decay (avgHF < 0.08)
  let fullAIScore = 0;
  if (!isAuthenticCamera) {
    if (isLossless && chroma.rRB < 0.985 && (smoothRatio > 0.40 || avgHF < 0.05)) {
      fullAIScore = Math.max(0.85, 0.98 - chroma.rRB * 0.15);
    } else if (chroma.rRB < 0.95) {
      fullAIScore = 0.88;
    }
  }
...
const ALL_BENCHMARK = [
  ...
  { type: "FULL AI", name: "AI Gen 1 (Jul 21)", path: "C:/Users/Gaurav Batule/Downloads/ChatGPT Image Jul 21, 2026, 10_48_58 AM.png", isAI: true },
  { type: "FULL AI", name: "AI Gen 2 (Jul 27)", path: "C:/Users/Gaurav Batule/Downloads/ChatGPT Image Jul 27, 2026, 10_04_53 AM.png", isAI: true },
  { type: "FULL AI", name: "AI Gen 3 (Jun 14)", path: "C:/Users/Gaurav Batule/Downloads/ChatGPT Image Jun 14, 2026, 10_53_14 PM.png", isAI: true },
  { type: "FULL AI", name: "AI Gen 4 (Jun 15)", path: "C:/Users/Gaurav Batule/Downloads/ChatGPT Image Jun 15, 2026, 12_20_13 AM.png", isAI: true },
  { type: "FULL AI", name: "AI Gen 5 (Jun 28)", path: "C:/Users/Gaurav Batule/Downloads/ChatGPT Image Jun 28, 2026, 02_50_17 PM.png", isAI: true },
  // 2. AI INPAINTINGS
...
async function run() {
  console.log("================================================================================");
  console.log("  TESTING 16 BENCHMARK CASES WITH FULL AI DIFFUSION SPECTRAL ENGINE");
  console.log("================================================================================\n");

  let passCount = 0;
  let totalCount = 0;

  for (const item of ALL_BENCHMARK) {
    ...
    const res = evaluateUnified(pixelRes, noiseRes, freqRes, imgData, info.width, info.height, isLossless, 'balanced');
