...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  // 2. Authentic Optical Camera Physics Check
  const isSyntheticChroma = chroma.rRB < 0.965 && avgNoiseVar > 450;
  const isAuthenticCamera = !isSyntheticChroma && (
    (!isLossless && chroma.rRB >= 0.970) ||
    bnc < -0.05 ||
    (avgNoiseVar < 300 && editRatio < 0.05)
  );

  // 3. Digital UI Screenshot Check
  const isDigitalUIScreenshot = exactFlatRatio > 0.50 || (isLossless && exactFlatRatio > 0.30 && editRatio < 0.028);

  // 4. Common Native AI Generation Resolutions
  const aiDimensions = [
  ...
  // 6. Dark Panel Infographic / Poster Geometry Check
