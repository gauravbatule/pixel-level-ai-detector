...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  // Authentic optical camera check: genuine low-variance photon shot noise across smooth surfaces
  const isAuthenticCamera = (avgNoiseVar < 350 && editRatio < 0.15) || (bnc < -0.05 && avgNoiseVar < 450 && editRatio < 0.15);

  // Digital UI Screenshot check: flat solid color UI components
  const isDigitalUIScreenshot = exactFlatRatio > 0.50 || (isLossless && exactFlatRatio > 0.35 && smoothRatio > 0.45 && avgNoiseVar < 500);
  ...
  if (!isDigitalUIScreenshot && !isAuthenticCamera) {
    ...
    } else if (chroma.rRB < 0.965 && avgNoiseVar > 500) {
