...
/**
 * Truly Universal, Color-Agnostic Pixel Forensics Engine
 * NO hardcoded color thresholds (no red, purple, yellow checks)
 * Uses pure mathematics:
 * 1. Inter-Channel Splice Boundary Gradients (Sobel on multi-channel differences)
 * 2. High-Pass Residual Inpainting Divergence (9x9 windowed variance)
 * 3. Diffusion Micro-Texture Discordance (Boundary sharpness vs Texture smoothness)
 * 4. Local Illuminant Vector Divergence (Illuminance physics)
 */
function performUniversalPixelForensics(imageData, width, height, isLossless = false, mode = 'balanced') {
  ...
  // --- 5. Pure Physical Fusion (Zero Color Bias) ---
...
function evaluateUnified(pixelRes, noiseRes, freqRes, imgData, rawW, rawH, isLossless = false, mode = 'balanced') {
  ...
  if (editRatio > (mode === 'strict' ? 0.02 : mode === 'normal' ? 0.04 : 0.03)) {
    ...
    if (isAuthenticCamera && editRatio < 0.12) {
      pixelScore *= 0.20;
    }
  } else {
...
async function run() {
  console.log("================================================================================");
  console.log("  TOTAL COMPREHENSIVE BENCHMARK: ZERO COLOR BIAS UNIVERSAL ENGINE");
  console.log("================================================================================\n");
