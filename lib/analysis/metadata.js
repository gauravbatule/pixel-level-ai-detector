...
function analyzeMetadata(meta, file, width, height) {
  ...
  // Missing EXIF is suspicious for JPEG
  if (!meta.exifFound && meta.isJpeg) {
    findings.push({ type: 'warning', message: 'No EXIF data found — common in AI-generated images', weight: 0.3 });
    suspicionScore += 0.3;
  }

  // PNG without EXIF + matching AI dimensions = mild suspicion
  if (meta.isPng && !meta.exifFound) {
    const commonAIDimensions = [256, 512, 768, 1024, 1536, 2048, 1080, 1920, 1344, 1792];
    const matchesDim = commonAIDimensions.includes(width) || commonAIDimensions.includes(height);
    ...
    if (matchesDim && matchesRatio) {
      findings.push({ type: 'warning', message: 'PNG with AI-typical dimensions and aspect ratio — no camera metadata', weight: 0.25 });
      suspicionScore += 0.25;
    } else if (matchesDim) {
      findings.push({ type: 'info', message: 'PNG with AI-common resolution — no camera metadata', weight: 0.1 });
      suspicionScore += 0.1;
    }
  }
