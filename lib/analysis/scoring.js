...
export function computeCompositeScore(elaResult, noiseResult, frequencyResult, cloneResult, metadataResult, pixelResult, mode = 'balanced', origWidth = null, origHeight = null, rawImageData = null) {
  ...
  if (!isAuthenticCamera) {
    // A. Lossless PNG with decoupled chroma residuals or high smoothness
    if (isLossless && (chroma.rRB < 0.985 || (smoothRatio > 0.70 && avgHF < 0.04))) {
      fullAIScore = Math.max(0.85, 0.98 - chroma.rRB * 0.12);
    }
    // B. JPEG with native AI resolution and high noise variance / latent smoothing
    else if (matchesAIDim && smoothRatio > 0.50 && avgNoiseVar > 300 && bnc >= 0.0) {
      fullAIScore = 0.88;
    }
    // C. Strong chromatic decoupled residual
