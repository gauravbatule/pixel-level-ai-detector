...
export function computeCompositeScore(elaResult, noiseResult, frequencyResult, cloneResult, metadataResult, pixelResult, mode = 'balanced', origWidth = null, origHeight = null, rawImageData = null) {
  ...
  const rawPData = rawImageData?.data;
  if (rawPData && rawPData.length >= 8) {
    const pLen = rawPData.length / 4;
    for (let i = 1; i < pLen; i++) {
      const idx = i * 4;
      const prev = (i - 1) * 4;
      if (rawPData[idx] === rawPData[prev] && rawPData[idx+1] === rawPData[prev+1] && rawPData[idx+2] === rawPData[prev+2]) {
        exactColorMatches++;
      }
    }
  }
  const exactFlatRatio = (rawPData && rawPData.length >= 8) ? exactColorMatches / (rawPData.length / 4) : 0;
  ...
  // Digital UI Screenshot Check (massive solid flat color runs or lossless app UI)
  const isDigitalUIScreenshot = exactFlatRatio > 0.50 || (isLossless && exactFlatRatio > 0.30 && (pixelResult?.stats?.editedAreaRatio ?? 0) < 0.028);
  ...
  if (rawImageData && rawImageData.data && numPix > 0) {
    const pData = rawImageData.data;
    for (let i = 0; i < numPix; i++) {
      const idx = i * 4;
      const lumVal = 0.299 * pData[idx] + 0.587 * pData[idx+1] + 0.114 * pData[idx+2];
      if (lumVal < 35) darkPanelCount++;
    }
  }
  const darkPanelRatio = numPix > 0 ? darkPanelCount / numPix : 0;
  const isAIInfographic = matchesAIDim && !isDigitalUIScreenshot && darkPanelRatio > 0.35 && smoothRatio > 0.50 && avgNoiseVar > 500 && avgHF > 0.015;

  // Genuine optical camera physics check (Bayer CFA residuals, photon shot noise, and natural noise floor)
  const isAuthenticCamera = !isAIInfographic && !hasEmbeddedPhotoInAI && !isDigitalUIScreenshot && (
    ...
    (avgNoiseVar < 280 && (pixelResult?.stats?.editedAreaRatio ?? 0) < 0.05)
  );
  ...
  if (pixelResult) {
    ...
    if (editRatio > (mode === 'strict' ? 0.02 : mode === 'normal' ? 0.04 : 0.03)) {
      ...
      if ((isAuthenticCamera && !isLossless) || (isDigitalUIScreenshot && editRatio < 0.05)) {
        pixelScore *= 0.15;
      }
    } else {
  ...
  if (!isDigitalUIScreenshot && !isAuthenticCamera) {
    if (isLossless && (chroma.rRB < 0.985 || (smoothRatio > 0.50 && avgNoiseVar > 500 && avgHF < 0.07))) {
      ...
      // Cross-channel residual decoupling with synthetic noise in PNG
      fullAIScore = 0.88;
